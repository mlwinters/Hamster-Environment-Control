...
1. Controls the automatic cage lighting to switch either the white or red lights on/off depending the time of day.<br/>
...
The systemreboottime value in housekeeping is self explanatory. When the system time is that value, it triggers a system restart. The pre-set time of just before midnight ensures that the last log entry has the correct time and that when the RasPi has finshed starting back up the new log file has correct date.<br/>
...
```
Once you've saved the changes, HEC will run on startup. To stop this, type
```
sudo nano /etc/rc.local
```
...
Don't use run cc-stream.py as a root user, VLC doesn't like it and won't start the stream.<br/>
...
cc-record.py saves camera data to a h264 video file for viewing and editing later. Unless your video player/editing software supports .h264 files. Videos will need to be converted into a MP4 file which can be done with MP4Box.<br/>
Both cc-stream and cc-record use the same camera settings and both reply on hec-status.ini to function.<br/>
I have also included the "-no-sensor" versions of cc-stream.py and cc-record.py. These are the same as the normal versions but without support for the BME280 incase you do not have the Adafruit BME280 temperature sensor.<br/>
